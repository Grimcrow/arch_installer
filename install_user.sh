...
cat /tmp/aur_queue | while read -r program 
do
	c=$(( "$c" + )) 
	dialog --infobox \
		"AUR install - Download and installing program $c out of $count: $program..." \
	10 60
	aur_check "$program"
done
...
if [ ! -d "$DOTFILES" ]; then
	git clone https://github.com/Grimcrow/dotfiles.git \
		"$DOTFILES" >/dev/null
fi
