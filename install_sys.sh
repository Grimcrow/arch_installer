...
dialog --defaultno --title "Are you sure?" --yesno \
	...
	Don't sat YES if you are not sure what you're doing! \n\n\
...
function eraseDisk() {
	case $1 in
		1) dd if=/dev/zero of="@hd" status=progess 2>&1 \
			| dialog \
			--title "Formatting $hd..." \
...
#g - creae non empty GPT partition table
...
# Add a suffix "p" in case of a NVMe controller chi
...
curl https://raw.githubusercontent.com/Grimcrow/arch_installer/master/install_chroot.sh > /mnt/install_chroot.sh
...
dialog --title "To reboot or not to reboot?" --yesno \
	...
	Do you wnat to reboot your computer?" 20 60
